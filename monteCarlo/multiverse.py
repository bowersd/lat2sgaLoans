...
phonotactics = [#what to look for in Latin
        ...
        re.compile('((?<=[aeiouAEIOU])(([tkdg](?![rlmn]))|[bm]))|((?<!n)[Ff])'), #lenition excluding compensatory lengthening, f, contextual carveout to allow cluster detection
...
if __name__ == "__main__":
    ...
    with open("aggregated_models_mean.csv", 'w') as file_out:
        file_out.write("period,mean,std\n")
        for i in range(len(means)+1): file_out.write(",".join((str(i), str(mean(*[meta_means[j][i] for j in range(len(meta_means))])), str(stdev(*[meta_means[j][i] for j in range(len(meta_means))]))))+'\n')
