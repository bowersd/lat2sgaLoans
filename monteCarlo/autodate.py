...
def check_procs(latin, irish):
    ...
    processes = [ #slightly refined regexen to apply to latin, paired with dicts to check if the rule applied or not. these need to be alignment-proof (overlook _)
            ...
            ((re.compile('((?<=[aeiouAEIOU])_*[tgms]|k(?![Tt]))|f'), { "t":"θð", "k":"xɣ", "g":"ɣx", "m":"ɱ", "s":"h", "f":"s"}), (re.compile('((?<=[aeiouAEIOU])_*(t|k|b|d|g|m|s(?![ptk])))|f'), {"t":"td", "k":"kg", "b":"b", "d":"d", "g":"g", "m":"m", "s":"s", "f":"f"})),
...
if __name__ == "__main__":
    ...
    for d in data:
        ...
        latin, irish = needleman.align(latin, irish, 1, needleman.read_similarity_matrix('simMatrix.txt'))
