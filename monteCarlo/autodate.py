...
processes = [ #slightly refined regexen to apply to latin, paired with dicts to check if the rule applied or not. these need to be alignment-proof (overlook _)
        ...
        ((re.compile('((?<=[aeiouAEIOU])_*[t]|(k)(?![Tt]))|f'), { "t":"θð", "k":"xɣ",  "f":"s"},1), (re.compile('((?<=[aeiouAEIOU])_*(t|k|b|d|g|m))'), {"t":"td", "k":"kg", "b":"b", "d":"d", "g":"g", "m":"m"},0)),
...
def procs_kludge(latin, irish, values):
    ...
    if any([irish[x.start():x.end()] == "f" for x in re.finditer("f", latin)]): values[1].append(3) #lenition legalized f
